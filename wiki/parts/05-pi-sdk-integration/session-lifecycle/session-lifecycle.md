...
- **Rule — session lock before session open.** The session-creation path acquires the sidecar lock before `SessionManager.open()` reads the session file. Reversing the order opens a race with another host / window. Acquisition goes through `_acquireSessionLock`, which reclaims a lock whose owner is provably gone — see [Part VII § writable-session-lock](../../07-safety-and-reversibility/writable-session-lock/writable-session-lock.md).
