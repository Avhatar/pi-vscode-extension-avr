...
- **Rule — blocking a tool call is rate-limited; injecting context is not.** The `tool_call` gate returns `{ block: true }` so the agent reviews newly matched directory-scoped resources and retries, and each block therefore costs a full model round-trip. The per-resource bookkeeping stops the *same* resource blocking twice, but not a run of *different* ones: a project with dozens of skills or rules can chain blocks until the agent looks hung, with nothing on screen naming the bridge as the cause. After `MAX_CONSECUTIVE_TOOL_BLOCKS` blocks in a row the gate stops blocking for the rest of the session ([context-extension.ts:289](../../../../src/pi/claude-compat/context-extension.ts#L289)); resources still flow, only the interruption stops. The counter resets on the first tool call that passes, and the trip deliberately survives compaction, because compaction clears the applied-resource entries and would otherwise restart the storm. A trip is reported through `onDiagnostic` to the Pi Code output channel and as a visible chat message.
