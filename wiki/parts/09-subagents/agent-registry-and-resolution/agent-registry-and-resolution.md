...
4. `resolveBoundedInteger(...)` [resolver.ts:250](../../../../src/pi/subagents/resolver.ts#L250) — `timeoutMinutes` resolves `invocation > definition > policy.default*`. A host ceiling (`policy.maxTurns`, `policy.maxTimeoutMinutes`) clamps the result and emits a `limit-clamped` diagnostic; when the ceiling is `undefined` the configured value is used verbatim. The host [session.ts:1233](../../../../src/pi/session.ts#L1233) deliberately passes **no** `maxTurns` ceiling, so `pi-code.subagents.defaultMaxTurns` is honoured at any size; `maxTimeoutMinutes` is still capped at `120`. Note that an agent definition's own limits outrank the setting — a definition that pins a small value makes the user setting inert for that agent.
5. `resolveTurnBudget(...)` [resolver.ts:248](../../../../src/pi/subagents/resolver.ts#L248) — `maxTurns` follows the same `invocation > definition > policy.default*` order with one asymmetry: **a definition's `maxTurns` is a floor, not a default.** An invocation may raise the budget but not lower it below the definition's number, and a lowered request is reported as a `limit-raised` diagnostic. The invocation value is still validated first, so an invalid request (`0`, negative, non-integer) fails rather than being silently replaced by the floor.
...
- **Rule — a definition's `maxTurns` is a floor the caller cannot undercut.** The invocation number is a per-call guess by the orchestrating model; the definition's number was written with the agent's real working style in mind. Every observed named-definition run had its declared budget overridden by a much smaller invocation value, which is what stranded the children. Raising is allowed, lowering is not.
