...
- **Rule — never await session bring-up inside `activate()`.** Providers, commands and subscriptions only take effect once `activate()` returns, so anything awaited before that point is time the Pi Code panel has nothing to draw — it reads as a frozen window, not as a slow start. `pi-code.prewarm.full` therefore dispatches `initialSession.initialize()` in the background and logs a failure rather than propagating it; the default lightweight branch, which runs the whole activation against an uninitialized session, is the proof that nothing downstream depends on it being ready.
