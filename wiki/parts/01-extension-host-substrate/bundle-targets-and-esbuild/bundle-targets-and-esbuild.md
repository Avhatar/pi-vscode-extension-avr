...
- **Pattern — narrow per-file `node_modules/` globs are safe.** Excluding a specific bundled-package asset (`node_modules/pi-web-access/banner.png`, `node_modules/pi-mcp-adapter/cli.js`) or a transitive test directory does not disturb the hoisted tree; it only shrinks the VSIX. Use `**/*.map` for source maps — a bare `*.map` matches root-level files only in vsce.
