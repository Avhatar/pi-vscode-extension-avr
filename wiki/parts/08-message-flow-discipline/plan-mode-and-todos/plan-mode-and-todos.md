...
- **Pitfall — "wait for approval" is guidance, not a gate.** The preamble tells the agent to stop after presenting a plan, but nothing enforces it: the active-tool set is untouched and a model can ignore the instruction. Enforcement was tried once and removed in 0.33.0 (`1defd6d`): a `plan`/`exec`/`idle` phase machine plus `setPlanModeActive` restricting tools to `PLAN_MODE_READONLY_TOOLS`, with the full set saved only in an in-memory `_savedToolNames`. It failed two ways — phase transitions depended on the model emitting `<plan-ready/>` / `<plan-complete/>` markers (never emitted ⇒ stuck), a 10-minute `EXEC_IDLE_RESET_MS` silently returned an idle chat to read-only, and any session swap lost the saved set, leaving edit tools permanently missing with no visible indication. Before reintroducing any gate, solve those three: no transition the user did not trigger, no destructive mutation of the active-tool set, and the current phase visible in the UI.
