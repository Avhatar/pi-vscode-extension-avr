...
- [src/webview/display-order.ts](../../../../src/webview/display-order.ts) — chronological placement of entries the transcript appends out of order
...
**Display order.** [`orderDisplayMessages`](../../../../src/webview/display-order.ts) is the single ordering pass behind `getDisplayMessageItems`. Transcript order is kept for everything except two kinds of entry that are appended later than the moment they describe: compaction summaries, and background subagent notifications, which are buffered while the parent turn streams and flushed at `agent_end`. Each is re-inserted before the first item that is chronologically later — notifications by `details.finishedAt`, compactions by their own timestamp. A notification with no `finishedAt` (transcripts written before that field existed) keeps its append position rather than being guessed into the wrong place. Repositioning is display-only; `sourceIndex` still points at the transcript entry, and the file on disk keeps an append order that remains a valid provider context.
...
- **Pitfall — do not "fix" out-of-order notifications by appending them earlier.** A subagent notification is a custom message that participates in the LLM context. Appending it when the child settles would drop it between an assistant message carrying tool calls and its tool results, which providers reject. Buffering to `agent_end` is required; the ordering is corrected at display time only.
