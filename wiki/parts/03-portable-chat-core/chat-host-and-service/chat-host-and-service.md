...
- [`handleEvent(tab, event)`](../../../../src/core/chat/chat-host.ts#L401) — the agent event bus; reduces via `ChatService`, applies chat-event-policy, dispatches queue, publishes state (state sync for `message_end` is deferred one microtask, see the pitfall below).
...
The turn snapshot carries three tool breakdowns, rendered as separately named sections so no total can be mistaken for another. `toolStats` is what the agent called itself, delegation wrapper included (*only orchestrator tools time*). `childToolStats` is what its delegated runs called, grouped by tool rather than by child (*only subagents tools time*). `combinedToolStats` is own calls **minus** the `subagent` wrapper plus `childToolStats` (*total with subagents tools time*); the wrapper is excluded because its wall clock *is* the child's run, so keeping it alongside the child rows would report the same seconds twice. The latter two are omitted when no child tool ran in the turn, since they would then repeat or duplicate `toolStats`. Non-tool time is reported too. `modelWaitMs` is the turn's provider round-trips plus agent-loop overhead, derived as turn duration minus `turnToolBusyMs` and minus in-turn compaction — a **union of tool-execution intervals**, not a sum, because Pi runs a message's tool calls in parallel by default and summed durations therefore exceed real elapsed time. `subagentStats` remains the per-run view (*subagents total time and status*), and it measures whole runs rather than tools: a delegated run's wall clock is mostly provider round-trips and child session startup, so `subagentNonToolTime` derives that remainder for display and the two subagent figures reconcile. The webview repeats that row under both the child tool breakdown and the per-run list, because either section read alone otherwise leaves the gap unexplained.

Turn metrics outlive the extension host. `_persistTurnMetrics` writes them into the session branch as a plain `custom` entry (`pi-code.turn-metrics`) — the SDK only turns `custom_message` entries into context messages, so this costs no tokens, stays out of the chat, and is destroyed with the session and by checkpoint rollback. `restoreTurnMetrics` replays them back into `messageMeta`, `toolDurations`, and the running `totalTurnDurationMs`. It also hands the persisted delegated-run rows back to the tab via `adoptSubagentStat` and re-fills `subagentTurnKey`, so the manager's next snapshot recognises those runs instead of minting fresh rows, and a child that settles after the reload restates the turn that owns it. A turn is re-written when a background child settles after it closed, and replay takes the newest write per key.

Delegated runs are accounted on a separate channel because child sessions never reach `reduceEvent`. [`recordSubagentToolEvent(tab, event)`](../../../../src/core/chat/chat-service.ts) consumes the subagent manager's namespaced child tool events and accumulates per-run tool time; [`syncSubagentRuns(tab, runs)`](../../../../src/core/chat/chat-service.ts) reconciles the manager's run list into per-run status and wall-clock duration, and decides turn attribution from each run's own `queuedAt` against `agentStartTime`. `completeAgentEnd` attaches the rows for runs spawned during that turn — by reference, so a background child that settles later updates the closed turn's summary in place. Failed and cancelled runs are reported like completed ones.

[`dispatchDirectPrompt(tab, request, callbacks)`](../../../../src/core/chat/chat-service.ts#L346) is the entry point for user prompts — parses `/compact` inline, increments `turnCounter`, arms the notification gate, invokes `_runUserPrompt`. Before any of that it consults `callbacks.isSessionBusy()`: a session that is compacting, or still wrapping up between `agent_end` and `agent_settled`, rejects a plain prompt outright, so the text is appended to the queue instead (`{ kind: 'queued' }`) and released by the normal dispatch gate. Attachments cannot ride the queue, which stores text only, so a busy prompt carrying them throws rather than silently dropping them. Streaming controls flow through [`dispatchStreamingCommand`](../../../../src/core/chat/chat-service.ts#L383) (abort / steer / follow-up). Queue lifecycle is [`applyQueueControl(tab, command)`](../../../../src/core/chat/chat-service.ts#L402), [`reserveQueuedDispatch()`](../../../../src/core/chat/chat-service.ts#L443), [`dispatchNextQueued()`](../../../../src/core/chat/chat-service.ts#L449).

[`buildState(tab, context)`](../../../../src/core/chat/chat-service.ts) is the projection layer. It produces `SerializedAgentState`: compact model-context messages, a latest-page full-transcript projection, model, tools, streaming / compacting flags, session metadata, context usage, file changes, cache mode, controls (todos, subagents, tool selection, feature toggles), pending tools, streaming buffers, queued messages. Runtime timing, per-turn tool and delegated-run statistics, and provider-cost metadata are keyed by message identity (`assistantMetaKey`) and stamped onto the compact context and the transcript independently; per-call tool durations are matched the same way by `toolCallId`. Every UI surface that needs "what does this tab look like right now?" calls it.
...
- `SubagentStatEntry` — one delegated run's status, wall-clock duration, and child tool time from [tool-timing.ts](../../../../src/shared/tool-timing.ts)
- `SubagentToolEvent`, `SubagentRunTiming` — structural views of the subagent manager's child tool events and run lifecycle, kept local to [chat-service.ts](../../../../src/core/chat/chat-service.ts) so the reducer stays free of `src/pi` imports
...
- `recordSubagentToolEvent(tab, event)`, `syncSubagentRuns(tab, runs)` — [chat-service.ts](../../../../src/core/chat/chat-service.ts)
- `assistantMetaKey(message)` — stable turn-metadata identity for one assistant message [chat-service.ts](../../../../src/core/chat/chat-service.ts)
- `restoreTurnMetrics(tab)`, `annotateTranscriptPage(tab, page)` — [chat-service.ts](../../../../src/core/chat/chat-service.ts)
- `TURN_METRICS_CUSTOM_TYPE`, `collectPersistedTurnMetrics`, `parsePersistedTurnMetrics` — [turn-metrics.ts](../../../../src/shared/turn-metrics.ts)
- `mergeToolStats(...breakdowns)`, `isDelegationToolName(name)` — [tool-timing.ts](../../../../src/shared/tool-timing.ts)
- `accumulateToolStat`, `toolStatEntries`, `toolStatDisplayName` — [tool-timing.ts](../../../../src/shared/tool-timing.ts)
- `subagentStatStatus`, `sortSubagentStats`, `totalSubagentStats`, `parseSubagentStatEntries` — [tool-timing.ts](../../../../src/shared/tool-timing.ts)
...
- **Rule — turn metadata is keyed by message identity, never by position.** `messageMeta` is keyed by `assistantMetaKey(message)` (the SDK-required `timestamp`), and `buildState` stamps the compact context and the transcript independently from that map. Position keying was the original design and it broke on compaction: the compact context is rewritten, so an ordinal recorded at turn end either stopped existing — dropping the turn footer, tok/s, and the tool breakdown — or came to name a different message and handed one card an unrelated older turn's numbers. Do not reintroduce ordinal keying or a "align the newest assistants from the end" pass.
- **Rule — restore strictly after `resetSessionProjection`.** That reset clears `messageMeta` and `toolDurations`, so calling `restoreTurnMetrics` before it silently wipes the history it just loaded. Both `newSession` and `loadSession` call it immediately after the reset, and `totalTurnDurationMs` is restored with it — otherwise the running total restarts at zero and a later turn reports a "turns total" smaller than the turn containing it.
- **Rule — every transcript delivery path annotates.** `buildState` ships the latest page, but a page fetched by scrolling back arrives through the `getTranscriptPage` command. Both go through `annotateTranscriptPage`; annotating only the first is how history lost its footers, breakdowns, and per-call timings.
- **Rule — pre-prompt compaction is charged for display but never subtracted.** Pi checks for context overflow both after an assistant message (inside the run) and before sending a prompt (outside it). `turnCompactionMs` covers the first and comes out of `modelWaitMs`; `pendingCompactionMs` covers the second, survives `agent_start`, and is reported with the turn it caused — but subtracting it would shrink the residual below the time that actually elapsed inside the turn.
- **Rule — non-tool time is measured, not subtracted from summed durations.** `turnToolBusyMs` accumulates only while `turnToolInFlight > 0`, so overlapping calls are counted once. Deriving the residual from `Σ toolStats.durationMs` would understate the wait whenever a message ran its tools in parallel, and could go negative. `completeAgentEnd` closes an interval left open by an abandoned tool at `turnEndAt` before computing the residual.
- **Rule — the delegation wrapper never appears in `combinedToolStats`.** `turnToolStats` accumulates every parent call; `turnDirectToolStats` accumulates the same minus `subagent`; `turnChildToolStats` accumulates the children's calls. The combined breakdown is built from the latter two, so the wrapper's seconds are represented once, by the child rows that actually did the work. Never build it by merging `turnToolStats` with child stats.
- **Rule — delegated-run timing must not force a state sync.** `recordSubagentToolEvent` and `syncSubagentRuns` only mutate the runtime. Publishing state per child tool event would re-project the whole transcript on every subagent keystroke; the rows are read live at the next natural sync (turn end, or the background-settle notification). Do not add a `publishState` call to these paths.
...
- **Pitfall — the session branch lags one append behind `message_end` delivery.** The SDK invokes its listeners (`_emit`) and only then persists the finalized message (`sessionManager.appendMessage` runs synchronously right after `_emit` in `_handleAgentEvent`). Reading the transcript from `sessionManager.getBranch()` during the `message_end` handler therefore misses the just-appended message. `ChatHost.handleEvent` defers the `message_end` name refresh and state publish to a `queueMicrotask` so the append lands first — without this, the user's own prompt would not appear in the chat (and the auto-derived tab title would not set) until the next state sync, i.e. the agent's first response.
