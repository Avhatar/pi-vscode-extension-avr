...
- Delegated-run timing: bounded `subagentStats: Map<agentId, SubagentStatEntry>` (rows mutated in place), `turnSubagentIds: Set<agentId>` (runs first seen during the current turn), `subagentTurnKey: Map<agentId, metaKey>` (which turn to re-persist when a run settles late), `pendingSubagentTools: Map<namespacedCallId, { startTime, name }>` (the name is resolved at tool start, where MCP-qualifying arguments are present).
...
- `recordMessageMeta(key, meta)` — stores one assistant message's turn metadata and evicts the oldest entry above `MESSAGE_META_HISTORY_LIMIT`
...
- `subagentStat(agentId, name?)` — upserts a delegated-run row without attributing it to any turn
- `adoptSubagentStat(entry)` — takes ownership of an existing row object (rows rebuilt from persisted metrics keep their stored identity) and evicts the oldest entry above `SUBAGENT_STAT_HISTORY_LIMIT`
- `chargeSubagentToTurn(agentId)` — attributes a run to the open turn; a no-op unless `isStreamingLocal`
...
- **Note — `messageMeta` and `toolDurations` are no longer empty at start.** They are rehydrated from the session branch by `ChatService.restoreTurnMetrics`; see [chat-host-and-service](../chat-host-and-service/chat-host-and-service.md).
...
- **Rule — a run is charged to the turn that spawned it, once.** Attribution is the run's own `queuedAt` (`startedAt` only as a fallback) compared against `agentStartTime`, decided in `ChatService.syncSubagentRuns` and applied through `chargeSubagentToTurn`. `queuedAt` and not `startedAt`, because a run that waited for a concurrency slot still belongs to the turn that delegated it. Restored history and runs observed while the tab is idle land in no turn at all.
- **Pitfall — first sight of an agent id is not evidence of a spawn.** `SubagentManager.onDidChange` replays the full snapshot to every new subscriber, and `retainTerminalRun` re-emits it whenever a terminal run ages out, so the entire retained history reaches `syncSubagentRuns` at arbitrary moments — including mid-turn, ten minutes after a reload. Charging on row creation therefore billed every restored run to whichever turn was streaming; that is why the decision moved to the timestamp. The one place first sight is still trusted is `recordSubagentToolEvent`, where an unknown id is already executing tools and restored runs come back terminal.
- **Rule — `turnChildToolStats` follows `turnSubagentIds`.** Child tool time is accumulated only for runs charged to the open turn, so `ONLY SUBAGENTS TOOLS TIME` and `SUBAGENTS TOTAL TIME AND STATUS` always describe the same set of runs. A background child from an earlier turn still updates its own row's `toolDurationMs`.
