...
- `subagentStat(agentId, name?)` — upserts a delegated-run row, charges it to the current turn only when `isStreamingLocal`, and evicts the oldest entry above `SUBAGENT_STAT_HISTORY_LIMIT`
