...
- [`shouldDispatchQueueAfterTerminal(eventType, state)`](../../../../src/core/chat/chat-event-policy.ts#L67) — true when the event is `agent_end` with `!isSessionStreaming`, `agent_settled` with `!isStreamingLocal`, OR `compaction_end` with neither flag set. The two-phase check exists because the SDK reports session-level and local-tab-level streaming separately; the `compaction_end` arm covers a compaction that runs *outside* an agent run (a direct `/compact`, or Pi's pre-prompt overflow check), which never reaches `agent_settled` and would otherwise strand the queue until some later turn ended.
...
- **Pitfall — a compaction is terminal only when no run is open.** `compaction_end` fires both inside a run (Pi's post-assistant overflow check, which lands *after* `agent_end` has already cleared `isStreamingLocal`) and outside one. Releasing the queue on the in-run kind would prompt a session that is still busy and get the message rejected, so both flags must be clear.
