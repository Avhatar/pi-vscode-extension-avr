...
import { DEEPSEEK_FLASH_MODEL_ID, registerDeepSeekFlashModel } from './providers/deepseek';
...
export async function prepareModelRuntime(
    ...
    syncDeepSeekFlashModel(runtime, log);
    await refreshModelMetadata(runtime, log);
    return runtime;
}

/**
 * Temporary catalog shim; see `registerDeepSeekFlashModel`. A failure here must
 * not cost the user their whole model list, so it is reported and swallowed.
 */
function syncDeepSeekFlashModel(runtime: ModelRuntime, log?: ModelMetadataLog): void {
    try {
        if (registerDeepSeekFlashModel(runtime)) {
            log?.(`DeepSeek V4.1 Flash added until the Pi SDK catalog ships it: ${DEEPSEEK_FLASH_MODEL_ID}`);
        }
    } catch (err) {
        console.error('[pi-code] Failed to register the DeepSeek V4.1 Flash model:', err);
    }
}
