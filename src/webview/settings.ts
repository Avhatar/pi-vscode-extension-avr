...
function render(data: SettingsData): void {
    ...
    container.appendChild(buildSection('Subagents', [
        ...
        buildToggle('subagents.allowChildBash', 'Allow shell access for children', data.subagentsAllowChildBash,
            'Grant children the bash tool. Worktree isolation bounds a child\'s edits, not what a shell can reach, so this is the parent\'s own machine access without the parent\'s review step. It also saves many turns: one git diff or rg replaces a long read/grep sequence.'),
        buildNumberInput('subagents.defaultMaxTurns', 'Default maximum turns', data.subagentsDefaultMaxTurns, 1, undefined,
            'Maximum child turns when neither the agent definition nor the invocation sets one. A turn is one child response including its tool calls, so tool-heavy agents consume many. A definition\'s own budget acts as a floor the caller cannot lower.'),
