...
- **Message queuing**: While streaming, user messages are queued in the tab runtime and auto-dispatched as new prompts on `agent_settled`. Do not dispatch from `agent_end`: the SDK still reports the session as busy until settlement, so a normal prompt will be rejected and lost. Compaction counts as busy for the same reason and on both sides: anything typed then is queued rather than prompted (`DirectPromptCallbacks.isSessionBusy()` is the authoritative check, since published state lags the session), and a compaction that ran *outside* a run releases the queue on `compaction_end`, because no `agent_settled` follows it. Steering (mid-stream injection) is a separate path via `AgentSession.steer()`.
