...
- **RawMode**: Opt-in per-chat developer view of every event and provider payload the agent exchanges with the model, gated globally by `pi-code.rawMode.enabled` (default off). Disabling the setting stops all capture channels immediately and hides the chat-panel Raw View icon; existing recordings remain available for explicit deletion. Storage is keyed by Pi `sessionPath` (the stable identity used across window reloads), not by tabId. Data lives in append-only JSONL under `context.globalStorageUri.fsPath/raw/<sha256>.jsonl`, and cleanup piggy-backs on `deleteHistorySession` (same call site as `_subagentStore.deleteByParentSessionPath`). Enabled recording is unbounded — no redaction and no retention policy; only user-initiated Clear removes data. Two capture channels feed the recorder: the inline Pi extension (`pi.on(...)` for harness-level events including `before_provider_request`, `after_provider_response`, `context`, and streamed `message_update` events) and `EventRouter.onAll(...)` for `AgentSession`-only events (`entry_appended`, `queue_update`, `compaction_*`, `auto_retry_*`, `thinking_level_changed`). The channels are recorded verbatim without dedup — the collated view groups them visually.
...
- **Wiki sync (proactive):** After a code change that adds / renames / removes an exported type, class, or controller name in a documented area, moves a type across namespaces referenced by an article, or contradicts a rule / pitfall stated in an existing article, run a quick `grep -r <identifier> wiki/` (or scan the owning article's Keywords) as part of the completion check. If any article references the affected identifier or rule, surface the divergence in chat and offer to invoke `wiki-maintain` before finishing the task — do not edit `wiki/**` silently, and never bundle wiki edits into the same commit as the code change unless the user asks for it. Trivial internal helpers, editor / build / test-only symbols, webview-only DOM edits, and one-off configuration touches do not need this check. This is guidance, not a gate.
