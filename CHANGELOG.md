...
- A message sent while the agent is compacting its context is now queued and delivered as soon as the agent is free, instead of being rejected with a raw SDK error ("Agent is already processing. Specify streamingBehavior...") and lost. The same applies to the short gap after a response finishes while the session is still wrapping up: anything typed there is queued rather than refused. The input placeholder says so during compaction, and Ctrl+Enter queues too, because there is no turn to steer into.
- One unusable stored API key no longer takes the rest down with it. Keys are applied provider by provider under a deadline, and a provider that cannot be configured — or that does not answer in time — is now reported and skipped instead of aborting the whole pass or stalling it. Previously every provider after it in the list was left unconfigured and the chat failed to start at all, and a provider stuck on the network could freeze every chat at once. The skipped provider is retried the next time keys are synchronized.
...
- The subagent section now reports `Queued before start`: time delegated runs spent waiting for a concurrency slot. Run totals begin when a run actually starts, so with the default limit of two concurrent children this wait could exceed the work itself and was visible only in the launcher.
